...
def check_valid_variable_assignment(pseudocode: str) -> tuple:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Assign): # True if node is assignment, i.e., "="
            for target in node.targets: # Iterates through each token in the assignment section of the code
                if isinstance(target, ast.Name): # Ensure that the token is a variable name
                    defined_variables.append((target.id, node.lineno)) # Record the variable in the list and its line number
