...
import { HomeIcon, EyeSlashIcon, EyeIcon, PlayIcon, TrashIcon } from "@heroicons/react/24/solid";
...
const CodeEditor: React.FC = () => {
  ...
  const getSectionWidths = () => {
    ...
    if (visibleSections === 2) {
      if (!showTask) return { taskW: "hidden", editorW: "w-3/4", outputW: "w-1/4" };
      if (!showOutput) return { taskW: "w-1/4", editorW: "w-3/4", outputW: "hidden" };
  ...
  const fetchFromBackend = async (input: string) => {
    if (code.trim()) {
      const response = await fetch("/api/compile", {
        ...
        body: JSON.stringify({ pseudocode: code, userInput: input }),
      });
  ...
  return (
    <div className="flex flex-col h-screen">
      {/* Navigation Bar */}
      <nav className="flex items-center justify-between bg-gray-800 text-white pt-4 pb-4">
        <div className={`nav-els ${taskW != "hidden" ? taskW : "w-1/5"}`}>
          <button
        ...
        <div className={`nav-els ${outputW != "hidden" ? outputW : "w-1/5"}`}>
          <button
            onClick={() => setShowOutput(!showOutput)}
            className={`nav-btns ${showOutput ? "bg-red-500" : "bg-green-500"}`}
          >
            {showOutput ? <EyeIcon className="nav-icons" /> : <EyeSlashIcon className="nav-icons" />}
          </button>
          <button
            onClick={() => setOutput([])}
            className="nav-btns bg-yellow-500 hover:bg-yellow-700"
          >
      ...
      {/* Main Content */}
      <div className="border-t-4 border-black flex flex-grow">
        {/* Task Description */}
        {showTask && (
          <div className={`bg-gray-800	flex flex-col ${taskW} p-4 overflow-y-auto`}>
        ...
        {/* Code Editor */}
        {showEditor && (
          <div className={`flex flex-col ${editorW} bg-gray-900 p-4 overflow-y-auto`}>
        ...
        {/* Output Section */}
        {showOutput && (
          <div className={`flex flex-col ${outputW} bg-gray-800 p-4 overflow-y-auto`}>
            ...
            {isComplete && <p className="text-green-600 mt-4">Execution Completed</p>}
          </div>
        )}
      </div>
    </div>
